...
uses
  ...
  //user created units
   PherMap, BatchForm, Ant, ComCtrls;          
...
var i,j,k, ant1, ant2, antsMeet, total: integer;
  tree1, tree2: TTree;
...
begin
  ...
  for i:= 0 to mCfg.nC -1 do begin //for the length of the cityCount array
    antsMeet := antCountList[i];
    if (antsMeet > 1) then begin   //more than 1 ant on the city
      for j:= 0 to antsMeet -2 do begin
        //ant indexes are given by antCityList[city][j]
        ant1  := antCityList[i][j];
        tree1 := TAnt(aL.Items[ant1]).GetTree;
        for k:= j+1 to antsMeet -1 do begin
          ant2 := antCityList[i][k];
