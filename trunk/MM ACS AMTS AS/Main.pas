  ...
  try
    ...
    if batch.ShowModal = mrOk then begin
      ...
      if(batch.AntCheck.Checked) then
        mBat.nAE := mBat.nAB;
      if(batch.AntFacCheck.Checked) or (mBat.aFType = AFNONE) then
        mBat.aFE := mBat.aFB;
      ...
      totalIter := mBat.iteration * allAF * allDir
...
begin
  ...
  WriteLn(txt,'ACS Algorithm format:');
  ...
  while(mCfg.aFType <> AFALL) do begin
...
var antL, mAL, iterL, curGen, i, curProgress: integer;
...
begin
  ...
  Rewrite(txt);
  WriteLn(txt,'BatchFile test, number of cities :=' +IntToStr(mCfg.nC));
  WriteLn(txt,'AMTS Algorithm - format:');
...
begin
  ...
  Rewrite(txt);
  WriteLn(txt,'BatchFile test, number of cities :=' +IntToStr(mCfg.nC));
  WriteLn(txt,'AS Algorithm - format:');
...
begin
  ...
  Rewrite(txt);
  WriteLn(txt,'BatchFile test, number of cities :=' +IntToStr(mCfg.nC));
  WriteLn(txt,'MM Algorithm  - format:');
  WriteLn(txt,'avgGen avgdist DP AP IP NA AF Path1D Path1C Path2D Path2C Path3D Path3C ANTMEET');
  // 8 loops- AFTYPE, AFDIR, number of ants, add P, decay P, init P, AntF, Iterations
  // start of with the number of ants
  if mBat.aFType = AFALL then
...
begin
  MessageDlg('Fuck you Baltimore!', mtInformation	,[mbOk],0);
end;
